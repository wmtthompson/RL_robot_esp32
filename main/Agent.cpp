...
void Agent::policy_lookup(const Scan_State &s1, Action &a1)
{
	...
	uint32_t greed_test_num = (esp_random() % 100);
	if (greed_test_num < m_greedyness)
	{
		m_chosen_state = BEST;
	}
	else
	...
	case RANDOM:
		...
		printf("## Choosing Random. \n");
		idx = static_cast<unsigned int>(esp_random() % 26);
		break;
	case BEST:
		printf("## Choosing the BEST. \n");
